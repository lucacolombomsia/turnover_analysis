...
def read_data(table_name):
    ...
    When asked to read the table that contains the training data, it will return a tuple
    with the data from all the predictors (the X matrix) and the data for the response (the Y vector).
    When asked to read a table that contains the test data, it will return a pandas dataframe
    with the data from all the predictors (the X matrix).
    ...
    #in this case, 
    #if the data doesn't contain the column "left", we have queried one of the test sets
    try:
        y = data.left
...
def preprocess_for_sklearn(data):
    """
    Preprocesses the data for all the predictors into a format that is compatible with sklearn,
    by converting all categorical variables into dummies.
    ...
    #these include: employer ID, categorical vars and one dummy per category (to avoid perfect multicollinearity)
